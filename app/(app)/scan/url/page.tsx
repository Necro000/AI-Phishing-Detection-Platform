...
function SignalBadge({ label, value }: { label: string; value: boolean | null | number | string }) {
  let display = '—'
    ...
    <main className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900 p-6">
      <div className="max-w-2xl mx-auto">
        ...
        <form onSubmit={handleScan} className="mb-8">
          <div className="flex gap-3">
            <input
              ...
              disabled={loading}
              className="flex-1 px-4 py-3 bg-white/10 border border-white/20 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50"
            />
            <button
              id="scan-url-btn"
              type="submit"
              disabled={loading || !url.trim()}
              className="px-6 py-3 bg-blue-600 hover:bg-blue-500 disabled:bg-blue-800 disabled:cursor-not-allowed text-white font-semibold rounded-xl transition-colors whitespace-nowrap"
            >
              {loading ? (
          ...
          {error && (
            <p role="alert" className="mt-3 text-red-400 text-sm">{error}</p>
          )}
        </form>
        ...
        {result && riskConfig && signals && (
          <div
            id="scan-result"
            className={`rounded-2xl border p-6 ${riskConfig.bg} ${riskConfig.border} animate-in fade-in duration-300`}
          >
            {/* Verdict header */}
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <SignalBadge label="Rule Score" value={signals.rules} />
                <SignalBadge label="Safe Browsing" value={signals.safeBrowsing} />
                <SignalBadge label="VirusTotal" value={signals.virusTotal} />
                <SignalBadge
                  label="ML Signal"
